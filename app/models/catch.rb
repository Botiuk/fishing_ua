...
class Catch < ApplicationRecord
  ...
  def self.all_day_catch_weight(fishing_session_id, day_rate_water_bioresource_ids)
    Catch.where(fishing_session_id: fishing_session_id, catch_result: 'pick_up', catch_time: DateTime.now.all_day)
         .where.not(water_bioresource_id: day_rate_water_bioresource_ids).sum(:catch_weight).to_s.to_f
  end

  def self.maximum_day_catch(fishing_session_id, day_rate_water_bioresource_ids)
    Catch.where(fishing_session_id: fishing_session_id, catch_result: 'pick_up', catch_time: DateTime.now.all_day)
         .where.not(water_bioresource_id: day_rate_water_bioresource_ids).maximum(:catch_weight).to_s.to_f
  end

  def self.all_day_catch_one_resource_weight(fishing_session_id, water_bioresource_id)
    Catch.where(fishing_session_id: fishing_session_id, catch_result: 'pick_up', catch_time: DateTime.now.all_day,
                water_bioresource_id: water_bioresource_id).sum(:catch_weight).to_s.to_f
  end

  def self.all_day_catch_one_resource_quantity(fishing_session_id, water_bioresource_id)
    Catch.where(fishing_session_id: fishing_session_id, catch_result: 'pick_up',
                catch_time: DateTime.now.all_day, water_bioresource_id: water_bioresource_id).count
  end

  def self.statistic_all_records(user_id, search_type, search_params)
    case search_type
    when 'water_bioresource'
      Catch.joins(:fishing_place).where(fishing_place: { user_id: user_id })
           .where(water_bioresource_id: search_params).order(:catch_time, :id).reverse_order
    when 'fishing_place'
  ...
  def self.statistic_max_weight(user_id, search_type, search_params)
    ...
    when 'water_bioresource'
      Catch.joins(:fishing_place).where(fishing_place: { user_id: user_id })
           .where(water_bioresource_id: search_params).maximum(:catch_weight)
    when 'fishing_place'
  ...
  def self.statistic_max_length(user_id, search_type, search_params)
    ...
    when 'water_bioresource'
      Catch.joins(:fishing_place).where(fishing_place: { user_id: user_id })
           .where(water_bioresource_id: search_params).maximum(:catch_length)
    when 'fishing_place'
