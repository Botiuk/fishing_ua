...
class RatePenaltiesController < ApplicationController
  ...
  def index
    @pagy, @rate_penalties = pagy(
      RatePenalty.includes(:water_bioresource).joins(:water_bioresource).order('water_bioresources.name'), items: 10
    )
  ...
  def water_bioresources_formhelper
    ...
    @water_bioresources = WaterBioresource.where.not(id: water_bioresources_with_penalty_ids).order(:name).pluck(
      :name, :id
    )
  end
